...
func SelectMovieById(movieId int) (model.Movie, error) {
	...
	row, err := dB.Query("SELECT id, name, year, director, screenwriter, starring, type, country, language, releasedate, length, aka, imdb, starnum, score, onestar, twostar, threestar, fourstar, fivestar, havewatched, wanttowatch, synopsis FROM movie WHERE id = ? ", movieId)
	if err != nil {
